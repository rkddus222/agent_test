...
def _build_select(
    ...
    # agg 레이어인 경우: 테이블 프리픽스 처리
    # - base_layers에 있는 테이블: alias 부여 (table.column AS table_column)
    # - base_layers에 없는 테이블: 프리픽스 제거
    if metrics and base_layers:
        metrics = add_table_prefix_to_columns(metrics, base_layers)

    # uppermost(deriv) 레이어인 경우: table.column → table_column 변환
    if metrics and from_name == "agg":
        metrics = remove_table_prefix_from_columns(metrics)

    # dimension → metric 순서로 정렬
    if metrics:
    ...
    if filters:
        ...
        for f in filters:
            if f.find(exp.Window) or (
                ...
                # OR 표현식을 괄호로 묶어서 AND 연산 시 우선순위 보존
                filter_expr = exp.Paren(this=f) if isinstance(f, exp.Or) else f
                where_condition = (
                    filter_expr
                    if where_condition is None
                    else exp.And(this=where_condition, expression=filter_expr)
                )
    ...
    # agg 레이어인 경우: base_layers에 없는 테이블 프리픽스 제거
    if groups and base_layers:
        groups = add_table_prefix_to_columns(groups, base_layers)

    # uppermost(deriv) 레이어인 경우: table.column → table_column 변환
    if groups and from_name == "agg":
        groups = remove_table_prefix_from_columns(groups)
...
def _sort_metrics_dimension_first(metrics: List[exp.Expression]) -> List[exp.Expression]:
    ...
    분류 기준:
    - dimension: 집계 함수(SUM, COUNT, AVG, MAX, MIN)나 윈도우 함수가 없는 표현식
    - metric: 집계 함수나 윈도우 함수가 포함된 표현식
    """
    dimensions = []
    aggregated_metrics = []
