...
from backend.semantic.model_manager.utils.ddl_types import TableInfo
# Parser imports moved to parse_ddl_text to avoid circular imports
...
def parse_ddl_text(ddl_text: str, dialect: Optional[str] = None) -> Dict[str, TableInfo]:
    """
    DDL SQL 텍스트를 파싱하여 테이블 정보를 추출합니다.
    
    Args:
        ddl_text: DDL SQL 텍스트
        dialect: DBMS 타입 (지정하지 않으면 DDL 텍스트 첫머리의 주석에서 자동 감지)
    ...
    Raises:
        DDLDialectError: dialect 주석이 없거나 지원하지 않는 dialect인 경우
    """
    # dialect가 지정되지 않은 경우 DDL 텍스트에서 추출
    if dialect is None:
    ...
    from backend.semantic.model_manager.utils.ddl_parsers import (
        PostgreSQLParser,
        BigQueryParser,
        MySQLParser,
        OracleParser,
