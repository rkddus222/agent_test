...
class PostgreSQLParser(BaseDDLParser):
    """PostgreSQL DDL 파서"""
    
    def parse_table_name(self, table_match: re.Match) -> Tuple[Optional[str], Optional[str], str]:
        """
        PostgreSQL 테이블명 파싱
        형식: CREATE TABLE [database.]schema.table 또는 CREATE TABLE schema.table
        """
    ...
    def parse_columns(self, table_block: str, table_info: TableInfo) -> None:
        ...
        # 컬럼 정의 패턴: column_name type [NULL|NOT NULL]
        ...
        # PRIMARY KEY, FOREIGN KEY 패턴 (스킵용)
        ...
        for line in lines:
            ...
            if col_name_match:
                ...
                # 타입은 공백 이후부터 쉼표, NULL, NOT NULL 전까지
                type_match = re.match(
                    r'^([A-Za-z0-9_()]+(?:\s+[A-Za-z0-9_()]+)*?)(?:\s*,\s*|\s+(?:NULL|NOT\s+NULL)|$)',
                    after_name,
                    re.IGNORECASE
                )
                ...
                if type_match:
                    ...
                    # 타입에서 뒤에 오는 NULL/NOT NULL 제거 (혹시 모를 경우)
                    col_type = re.sub(r'\s+(?:NULL|NOT\s+NULL).*$', '', col_type, flags=re.IGNORECASE)
