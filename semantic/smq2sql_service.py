...
from semantic.composer.pipeline.add_default_join import JoinError
from semantic.utils.metadata import collect_metadata_from_sql
from semantic.types.metric_type import load_metrics
from semantic.parser import SMQParser
from semantic.composer import SQLComposer
from semantic.utils.inline_converter import conver_cte_to_inline
from semantic.utils.distribute_smq import distribute_smq_with_designated_models
    ...
    try:
        ...
        if not isinstance(result, list):
            ...
            if result.get("success"):
                ...
                logger.error(
                    "  ❌ Result marked as unsuccessful: %s", result.get("error")
                )
...
def smq_to_sql(
    ...
    try:
        ...
        logger.info("✅ SqlComposer created sql: %s", sql)
        ...
        return {
            "success": True,
            "sql": sql.sql(dialect=dialect, pretty=True),
            "metadata": metadata,
        }
